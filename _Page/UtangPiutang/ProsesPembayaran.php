...
try {
    ...
    if ($isJualBeli) {
        ...
        $settingKategori = in_array($kategoriTransaksi, ['Penjualan', 'Retur Penjualan'], true) ? 'Penjualan' : 'Pembelian';
        $stmt = $Conn->prepare('SELECT debet, utang_piutang FROM setting_autojurnal_jual_beli WHERE kategori = ? LIMIT 1');
        ...
        $stmt = $Conn->prepare(
            'SELECT t.id_transaksi, t.jumlah, t.pembayaran, t.status, tj.kategori,
                    tj.id_akun_debet, tj.id_akun_kredit
        ...
        $akunKredit = $transaksi['id_akun_kredit'];
    ...
    $kategoriPembayaran = $jumlah == $sisaTagihan ? 'Pelunasan' : 'Termin';
        $kategoriPembayaranTransaksi = $isJualBeli
            ? $kategoriTransaksi
            : ($kategoriTransaksi === 'Pengeluaran'
    ...
    $stmt = $Conn->prepare(
        ...
        (id_transaksi, id_transaksi_jual_beli, kategori_pembayaran, kategori_transaksi,
