<?php
    ...
    for ( $i =$a; $i<=$b; $i++ ){
        ...
        //Jumlah Simpanan
        $SumPenjualan = mysqli_fetch_array(mysqli_query($Conn, "SELECT SUM(total) AS total FROM transaksi_jual_beli WHERE kategori='Penjualan' AND tanggal like '%$WaktuPencarian%'"));
        $DataPenjualan = $SumPenjualan['total'];
        $JumlahPenjualan = "" . number_format($DataPenjualan,0,',','.');
