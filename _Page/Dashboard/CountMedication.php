...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    try {
        ...
        if (empty($SessionIdAkses)) {
            ...
            $result = $Conn->query('SELECT COUNT(*) AS total FROM medication');
            $row = $result->fetch_assoc();
            $result->free();

            $response = [
                'status' => 'Success',
                'message' => 'Jumlah medication berhasil dihitung.',
                'count_medication' => number_format((int) $row['total'], 0, ',', '.')
            ];
        }
    } catch (mysqli_sql_exception $e) {
        $response['message'] = 'Terjadi kesalahan database saat menghitung jumlah medication.';
    }
}
