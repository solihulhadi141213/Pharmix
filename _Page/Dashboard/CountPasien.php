...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    try {
        ...
        if (empty($SessionIdAkses)) {
            ...
            $response = [
                'status' => 'Success',
                'message' => 'Jumlah pasien berhasil dihitung.',
                'count_pasien' => number_format((int) $row['total'], 0, ',', '.')
            ];
        }
    } catch (mysqli_sql_exception $e) {
        $response['message'] = 'Terjadi kesalahan database saat menghitung jumlah pasien.';
    }
}
