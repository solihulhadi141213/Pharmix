<?php
    ...
    if (empty($SessionIdAkses)) {
        ...
        // Kondisi jika id_supplier_baru kosong
        if (empty($_POST['id_supplier_baru'])) {
            $id_supplier = "NULL"; // Nilai NULL sebagai string
        } else {
            $id_supplier = "'" . mysqli_real_escape_string($Conn, $_POST['id_supplier_baru']) . "'"; // Escape input untuk keamanan
        }

        // Proses Update Transaksi
        $query = "UPDATE transaksi_jual_beli SET 
        id_supplier = $id_supplier, 
        update_by_id='$SessionIdAkses', 
        update_by_name='$SessionNama', 
        update_at='$now' 
        WHERE id_transaksi_jual_beli = '$id_transaksi_jual_beli'";
