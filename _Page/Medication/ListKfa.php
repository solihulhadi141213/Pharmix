<?php
    ...
    // Validasi Parameter
    // ===============================
    if (empty($_GET['medication_category'])) {
        echo json_encode(['results' => [], 'pagination' => ['more' => false]]);
        exit;
    }
    ...
    foreach ($items as $row) {
        ...
        $results[] = [
            ...
            'text'     => $row['kfa_code'] . ' - ' . $row['name'], // Fallback text standard
            'kfa_code' => $row['kfa_code'],                        // Data terpisah untuk template
            'name'     => $row['name']                             // Data terpisah untuk template
        ];
    }
