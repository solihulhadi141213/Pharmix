...
$page = filter_var($_POST['page_resep'] ?? 1, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]) ?: 1;
$batas = 5;
try {
    $stmt = mysqli_prepare($Conn, 'SELECT COUNT(*) AS jumlah FROM medication_request_group WHERE id_anggota = ?');
    if (!$stmt) throw new RuntimeException('Gagal menyiapkan jumlah resep.');
    mysqli_stmt_bind_param($stmt, 'i', $id_anggota);
    if (!mysqli_stmt_execute($stmt)) throw new RuntimeException('Gagal menghitung resep.');
    ...
    $posisi = ($page - 1) * $batas;
    $stmt = mysqli_prepare($Conn, 'SELECT id_medication_request_group, datetime_creat, dokter_nama, no_resep_nasional, sumber_resep, priority, status_resep FROM medication_request_group WHERE id_anggota = ? ORDER BY datetime_creat DESC, id_medication_request_group DESC LIMIT ?, ?');
    if (!$stmt) throw new RuntimeException('Gagal menyiapkan daftar resep.');
    mysqli_stmt_bind_param($stmt, 'iii', $id_anggota, $posisi, $batas);
    if (!mysqli_stmt_execute($stmt)) throw new RuntimeException('Gagal membaca resep.');
    $result = mysqli_stmt_get_result($stmt);
    $statusMap = ['Draft' => ['DRF', 'bg-secondary'], 'Verified' => ['VRF', 'bg-primary'], 'Partially' => ['PRT', 'bg-warning text-dark'], 'Completed' => ['CMP', 'bg-success'], 'Cancelled' => ['CNL', 'bg-danger']];
    $html = '<ul class="list-group list-group-flush">';
    while ($row = mysqli_fetch_assoc($result)) {
        $timestamp = empty($row['datetime_creat']) ? false : strtotime($row['datetime_creat']);
        ...
        $html .= barisRiwayat('Dokter', $row['dokter_nama']);
        $html .= barisRiwayat('No. Resep', $row['no_resep_nasional'], true);
        $html .= barisRiwayat('Sumber Resep', $row['sumber_resep']);
        $html .= barisRiwayat('Prioritas', strtoupper($row['priority'] ?? '-'));
        $html .= '</div></li>';
    }
    $html .= '</ul>';
    mysqli_stmt_close($stmt);
