<?php
    ...
    $OrderBy    = isset($_POST['OrderBy']) ? trim($_POST['OrderBy']) : 'd.judul';
    ...
    $tags = isset($_POST['tags']) && is_string($_POST['tags']) ? trim($_POST['tags']) : '';
    if ($tags !== '') {
        $where[] = 'EXISTS (
            SELECT 1 FROM dokumentasi_tags dt_selected
            WHERE dt_selected.id_dokumentasi = d.id_dokumentasi
            AND TRIM(dt_selected.tags) = ?
        )';
        $params[] = $tags;
        $types .= 's';
    }
    ...
    if ($keyword !== '') {
        //------ Filter Tags
        if ($keyword_by === '') {
            $where[] = '(d.judul LIKE ? OR d.deskripsi LIKE ?)';
            $params[] = '%' . $keyword . '%';
            $params[] = '%' . $keyword . '%';
            $types .= 'ss';
        } elseif ($keyword_by === 'tags') {
            $where[] = "
                EXISTS (
                    SELECT 1
    ...
    $html = '';
    $no = $offset + 1;
    $ringkas = static function ($value, int $limit): string {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $judul_ringkas = $ringkas($row['judul'] ?? '', 90);
            $deskripsi = $ringkas($row['deskripsi'] ?? '', 220);
            ...
            // Tabel belum memiliki tanggal terbit khusus; gunakan tanggal pembuatan.
            $label_tanggal = $status === 'Publish' ? 'Terbit' : 'Dibuat';
            //------ Render ringkasan bantuan
            $html .= '
                <article class="card bantuan-item mb-3 lihat_detail_bantuan" data-id="'.$id_dokumentasi.'" role="button" tabindex="0" style="cursor: pointer;">
                    <div class="card-body p-3 p-md-4">
                        <div class="d-flex justify-content-between align-items-start gap-3 mb-2">
                            <h3 class="h6 fw-bold text-break mb-0" title="'.$judul.'">'.$judul_ringkas.'</h3>
                            '.$html_status.'
                        </div>
                        <div class="small d-flex flex-wrap gap-3 text-muted mb-3">
                            <span><i class="bi bi-person me-1" aria-hidden="true"></i>'.$author_name.'</span>
                            <span><i class="bi bi-calendar3 me-1" aria-hidden="true"></i>'.$label_tanggal.': '.$tanggal.'</span>
                        </div>
                        <p class="small text-muted text-break bantuan-description mb-3">'.$deskripsi.'</p>
                        <div class="d-flex flex-wrap gap-1" aria-label="Tags bantuan">'.$html_tags.'</div>
                    </div>
                </article>
            ';
            $no++;
        ...
        $html = '
            <div class="alert alert-warning text-center">
                    <i class="bi bi-exclamation-triangle"></i><br>
                    <small>Tidak ada data dokumentasi.</small>
                </div>
        ';
    }
